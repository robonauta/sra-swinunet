...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        sr (nn.Module, optional): Spatial reduction module. Default: None
        norm (nn.Module, optional): Normalization module. Default: None
    """
    ...
    def forward(self, x, mask=None):
        ...
        if self.sr is not None:
            x_ = x.permute(0, 2, 1).reshape(B_, C, *self.window_size)
            x_ = self.sr(x_).reshape(B_, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            kv = (
                self.kv(x_)
                .reshape(B_, -1, 2, self.num_heads, C // self.num_heads)
                .permute(2, 0, 3, 1, 4)
            )
        else:
            kv = (
    ...
    def flops(self, N):
        ...
        if sr_ratio > 1:
            ...
            # --- FLOPs for the SRA specific layers ---
            # 1. FLOPs for the sr convolution (depth-wise): N * dim
            flops += N * self.dim
            # 2. FLOPs for the norm layer after sr: N_kv * dim
            flops += N_kv * self.dim
            ...
            # kv = self.kv(x_) -> x_ has N_kv tokens
            flops += N_kv * self.dim * 2 * self.dim

            # attn = (q @ k.transpose(-2, -1))
            # q: (H, N, C/H), k: (H, N_kv, C/H) -> attn: (H, N, N_kv)
            flops += self.num_heads * N * (self.dim // self.num_heads) * N_kv

            #  x = (attn @ v)
            # attn: (H, N, N_kv), v: (H, N_kv, C/H) -> x: (H, N, C/H)
            flops += self.num_heads * N * N_kv * (self.dim // self.num_heads)

        else:
            # Original non-SRA case
            # qkv = self.qkv(x)
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        if sr_ratio > 1:
            self.sr = nn.Conv2d(
                dim, dim, kernel_size=sr_ratio, stride=sr_ratio, groups=dim
            )
